...
pub async fn maker_polls_sanity_check(db: Arc<prisma::PrismaClient>, rpc: Arc<Provider<Http>>) {
    let sanitize_from: chrono::DateTime<Utc> = Utc::now() - Duration::from(Duration::days(90));
    let sanitize_to: chrono::DateTime<Utc> = Utc::now() - Duration::from(Duration::minutes(15));
