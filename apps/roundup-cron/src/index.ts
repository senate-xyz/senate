...
if (!process.env.POSTMARK_TOKEN) process.exit();
...
const addNewProposals = async (lastMonthProposals: Proposal[]) => {
  ...
  let proposals = lastMonthProposals.filter(
...
const addEndingProposals = async (lastMonthProposals: Proposal[]) => {
  ...
  for (let proposal of proposals) {
    let subscriptions: SubscriptionWithProxies[] =
    ...
    for (let subscription of subscriptions) {
      let userHasVoted = await userVoted(
