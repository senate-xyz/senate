...
const fetchProposalInfoFromIPFS = async (hexHash : string) => {
  ...
  try {
    const response = await axios.get('https://gateway.pinata.cloud/ipfs/f01701220' + hexHash.substring(2));
...
const getProposalTitleAndDescription = async (daoAddress: string, text: string) : Promise<any> => {  

    if (daoAddress === "0xEC568fffba86c094cf06b22134B23074DFE2252c") { // Aave 
...
const findGovernorBravoProposals = async (dao : any) => {
  ...
  const logs = await provider.getLogs({
    ...
    address: dao.address,
    topics: [
      govBravoIface.getEventTopic("ProposalCreated")
    ],
  });
  ...
  const ongoingProposals = proposals.filter(proposal => proposal.eventData.endBlock > latestBlockMined);
  ...
  for (let i=0; i<proposals.length; i++) {
    ...
    let votingStartsTimestamp = proposalCreatedTimestamp + (proposals[i].eventData.startBlock - proposals[i].txBlock) * 15;
    let votingEndsTimestamp = proposalCreatedTimestamp + (proposals[i].eventData.endBlock - proposals[i].txBlock) * 15;
    let {title, description} = await getProposalTitleAndDescription(dao.address, proposals[i].eventData[11]);
